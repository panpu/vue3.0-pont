...
import { Popover, Tree } from 'element-ui'
...
export default defineComponent({
  ...
  data() {
    return {
      propsValue: 'flowId',
      propsLabel: 'name',
      propsCode: null as any, // 可能有空的情况
      propsDisabled: 'disabled',
      propsChildren: 'children',
      data: [] as any[],
      keywords: '',
      labels: '' as string | string[], // 存储名称，用于下拉框显示内容
      ids: [] as any, // 存储id
      selectNodes: [], // 选中数据
  ...
  methods: {
    // _renderContent(h,row){
    //   if(this.renderContent){
    //     return this.renderContent(row)
    //   }else{
    //     return <span>{row.data[this.treeParams.props.value]} </span>
    //   }
    // },
    // 根据类型判断单选，多选
    _setMultipleFun() {
    ...
    _setSelectNodeFun(ids) {
      const el = this.$refs.tree as Tree
      if (!el) {
    ...
    _updatePopoverLocationFun() {
      // dom高度还没有更新，做一个延迟
      setTimeout(() => {
        ;(this.$refs.popover as any).updatePopper()
      }, 50)
    },
    ...
    _treeNodeClickFun(data, node, vm) {
      ...
      const { propsValue, propsChildren, propsDisabled, expandClick } = this
    ...
    _selectRemoveTag(tag) {
      ...
      ;(this.$refs.tree as Tree).setCheckedKeys(this.ids)
    ...
    _updateH() {
      this.$nextTick(() => {
        this.width = (this.$refs.select as any).$el.getBoundingClientRect().width
      })
    },
