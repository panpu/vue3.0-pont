...
export default defineComponent({
  ...
  setup(props, { listeners, emit }) {
    ...
    onMounted(async () => {
      if (props.id) {
        const res = await API.manageCenterServer.user.queryInfoById.request({ userId: props.id! })
        if (res.code) {
          const data = res.data!
    ...
    const formItems: IFormItem[] = [
      ...
      },
      {
        ...
        label: '用户名',
        required: true,
        render: () => <el-input disabled={!!model.id} v-model={model.account} />
      },
      {
        prop: 'userType',
        label: '用户类型',
      ...
      },
      {
        ...
        label: '所属部门',
        required: true,
        render: () => <TreeSelect selectParams={selectParams} v-model={model.deptIdList} treeParams={treeParams} />
      },
      {
        prop: 'email',
        label: '邮箱',
      ...
      },
      {
        prop: 'powerIdList',
        label: '审核权限',
        render: () => <TreeSelect selectParams={selectParams} v-model={model.powerIdList} treeParams={treeParams} />
      },
      {
        prop: 'visibleIdList',
        label: '可见部门',
        render: () => <TreeSelect selectParams={selectParams} v-model={model.visibleIdList} treeParams={treeParams} />
      }
    ]
