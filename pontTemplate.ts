...
export default class MyGenerator extends CodeGenerator {
  ...
  getInterfaceContent(inter: Interface) {
    ...
    const url = `/${this.dataSource.name.replace(/([A-Z])/g, '-$1').toLowerCase()}${inter.path}`
    return `
    /**
    ...
    import {fetch , getUrl} from '@/utils/request';
    ...
    export function request(${requestParams}) {
      return fetch(getUrl("${url}", params, "${method}"), ${inter.getRequestContent()});
    }
   `
  }
}
